...
def run(opt):
    ...
    rec_tracker = tf.keras.metrics.Mean(name='rec_loss')

    val_loss_tracker = tf.keras.metrics.Mean(name='val_loss')
    val_rec_tracker = tf.keras.metrics.Mean(name='val_rec_loss')

    train_metric = tf.keras.metrics.Mean(name='DE')
    test_metric = tf.keras.metrics.Mean(name='test_DE')
    test_rec_tracker = tf.keras.metrics.Mean(name='test_rec_loss')
    ...
    history = pd.DataFrame(columns=['epoch', 'loss', 'validation_loss', 'DE', 'validation_DE', 'test_DE', 'rec_loss', 'valid_rec_loss', 'test_rec_loss'])
    ...
    for epoch in range(stopped, epochs):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            with tf.GradientTape() as tape:
                ...
                logits, z, z_rec = model([x_batch_train[:, i] for i in get_random_k_view(n_views)])
                rec_loss = loss_fn(z, z_rec)
                loss_value = loss_fn(y_batch_train, logits)
                total_loss = loss_value + rec_loss
            ...
            rec_tracker.update_state(rec_loss)
            train_metric.update_state(delta_e_2000(y_batch_train, logits))

        # Display metrics at the end of each epoch.
        train_loss = loss_tracker.result()
        train_rec_loss = rec_tracker.result()
        train_de = train_metric.result()

        print("Training loss over epoch: {:.2f}, Training DE: {:.2f}, Rec loss: {:.2f}".format( float(train_loss), float(train_de), float(train_rec_loss)))

        # Reset training metrics at the end of each epoch
        loss_tracker.reset_states()
        train_metric.reset_states()
        rec_tracker.reset_state()
        ...
        for x_batch_val, y_batch_val in validation_dataset:
            ...
            # testing position indipendence and random views
            val_logits, z, z_rec = model([x_batch_val[:, i] for i in get_random_k_view(n_views)], training=False)

            # Update val metrics
            val_loss_tracker.update_state(loss_fn(y_batch_val, val_logits))
            val_metric.update_state(delta_e_2000(y_batch_val, val_logits))
            val_rec_tracker.update_state(loss_fn(z, z_rec))

        val_loss = val_loss_tracker.result()
        val_de = val_metric.result()
        val_rec_loss = val_rec_tracker.result()

        val_loss_tracker.reset_states()
        val_metric.reset_states()
        val_rec_tracker.reset_states()

        print("Validation loss over epoch: {:.2f}, Validation DE: {:.2f}, Rec loss: {:.2f}".format( float(val_loss), float(val_de), float(val_rec_loss)))
        ...
        if opt.test == 2 or (opt.test == 1 and epoch + 1 == epochs):
            ...
            for x_batch_test, y_batch_test in test_dataset:
                ...
                # testing position indipendence and random views
                test_logits, z, z_rec = model([x_batch_test[:, i] for i in get_random_k_view(n_views)], training=False)


                # Update val metrics
                test_metric.update_state(delta_e_2000(y_batch_test, test_logits))
                test_rec_tracker.update_state(loss_fn(z, z_rec))

            test_de = test_metric.result()
            test_metric.reset_states()

            test_rec_loss = test_rec_tracker.result()
            test_rec_tracker.reset_states()

            history = history.append({'epoch':epoch, 'loss': train_loss.numpy() , 'validation_loss':val_loss.numpy(), 'DE':train_de.numpy(), 'validation_DE':val_de.numpy(), 'test_DE': test_de.numpy(),
            'rec_loss':train_rec_loss, 'valid_rec_loss':val_rec_loss, 'test_rec_loss':test_rec_loss}, ignore_index=True)
            
            print("Test DE: {:.2f}, Rec loss: {:.2f}".format( float(test_de), float(test_rec_loss)))
